...
class OverlapPatchEmbed_Stride(nn.Module):
    """ Image to Patch Embedding
        x: B,C1,H,W
        return: B,h*w,C2
        process: conv + layernorm (C2)   resolution down by 4x
...
class Downsample(nn.Module):
    ...
    x: B,C,H,W
    return: B,2C,H/2,W/2
    process: conv + pixel-unshuffle (C down, resolution up)
    Adopted: Restormer
    """
...
class Upsample(nn.Module):
    ...
    x: B,C,H,W
    return: B,C/2,2H,2W
    process: conv + pixel-shuffle (C up, resolution down)
    Adopted: Restormer
    """
    def __init__(self, n_feat):
...
class Upsample_BCHW(nn.Module):
    ...
    x: B,C,H,W
    return: B,2C,h/2,w/2
    process: conv + pixel-shuffle (C up, resolution down); resolution doubles, channels halve
    Adopted: Restormer
    """
